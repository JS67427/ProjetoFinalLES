...
from .forms import QuestionarioForm, PerguntaForm
from .models import EstadoQuestionario
from .forms import EstadoQuestionarioForm
...
def listar_questionarios(request):
    ...
    ano_letivo_selecionado = request.GET.get('ano_letivo')
    assunto = request.GET.get('assunto')  # Adicionando a obtenção do termo de pesquisa por assunto

    if ano_letivo_selecionado:
        questionarios = Questionario.objects.filter(id_ano_letivo=ano_letivo_selecionado)
    else:
        questionarios = Questionario.objects.all()

    # Filtrando os questionários pelo assunto se o termo de pesquisa estiver presente
    if assunto:
        questionarios = questionarios.filter(assunto__icontains=assunto)

    context = {
        'questionarios': questionarios,
        'anos_letivos': anos_letivos,
        'ano_letivo_selecionado': int(ano_letivo_selecionado) if ano_letivo_selecionado else None,
    }

    return render(request, 'pedido/listar_questionarios.html', context)
...
def criar_questionario(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            pergunta_index = 1
            while f'pergunta_{pergunta_index}' in request.POST:
                texto = request.POST.get(f'pergunta_{pergunta_index}')
                tipo = request.POST.get(f'selected_option_{pergunta_index}')
                opcao1 = request.POST.get(f'pergunta_{pergunta_index}_opcao_1', None)
                opcao2 = request.POST.get(f'pergunta_{pergunta_index}_opcao_2', None)
                opcao3 = request.POST.get(f'pergunta_{pergunta_index}_opcao_3', None)
                opcao4 = request.POST.get(f'pergunta_{pergunta_index}_opcao_4', None)
                opcao5 = request.POST.get(f'pergunta_{pergunta_index}_opcao_5', None)
                opcao6 = request.POST.get(f'pergunta_{pergunta_index}_opcao_6', None)
                tema_id = request.POST.get(f'pergunta_{pergunta_index}_tema')
            ...
            # Salvar o ano letivo selecionado junto com o questionário
            ano_letivo_id = request.POST.get('ano_letivo')
            questionario.id_ano_letivo = ano_letivo_id
            questionario.save()

            return redirect('/../listar-questionarios/')  # Redirecionar para uma página de sucesso ou outra página relevante
    else:
        form = QuestionarioForm()
    
    return render(request, 'pedido/criar_questionario.html', {'form': form, 'temas': temas, 'anos_letivos': anos_letivos})



def consultar_questionario(request, questionario_id):
    # Obter o questionário pelo ID
    questionario = get_object_or_404(Questionario, pk=questionario_id)

    # Renderizar o template com os dados do questionário
    return render(request, 'pedido/consultar_questionario.html', {'questionario': questionario})

def validar_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)
    questionario.valido = True
    questionario.save()
    return redirect('/../listar-questionarios/')
...
def criar_tema(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            nome_tema = form.cleaned_data['tema']  # Supondo que o campo que define unicidade seja 'nome'
            if Tema.objects.filter(tema=nome_tema).exists():
                erros = "Não é possível editar o tema porque existem questionários associados."
                msg = True
                return render(request, 'pedido/editar_tema.html', 
                        context={'erros': erros,'msg':msg,'tema':nome_tema,})
            else:
                # Salvar o tema no banco de dados
                form.save()
                # Redirecionar para a lista de temas
                messages.success(request, 'Tema criado com sucesso!')
                return redirect('/listar-temas/')
    else:
        # Se não for um POST, criar um novo formulário em branco
        form = TemaForm()
    
    return render(request, 'pedido/criar_tema.html', {'tema': form})


def listar_temas(request):
    search_query = request.GET.get('search', '')
    if search_query:
        temas = Tema.objects.filter(tema__icontains=search_query)
    else:
        temas = Tema.objects.all()
    ...
    return render(request, 'pedido/listar_temas.html', context)
...
def eliminar_tema(request, pk):
    ...
    # Verificar se existem questionários associados a este tema
    if Pergunta.objects.filter(tema=tema).exists():
        erros = "Não é possível eliminar o tema porque existem questionários associados."
        msg = True
        temas = Tema.objects.all()
        return render(request, 'pedido/listar_temas.html', context={'erros': erros,'msg':msg,'temas':temas,})
    ...
    if request.method == 'POST':
        if 'confirmar' in request.POST:
            ...
            return redirect('/../listar-temas/')
    return render(request, 'pedido/eliminar_tema.html', {'tema': tema})
...
def editar_tema(request, pk):
    ...
    if request.method == 'POST':
        ...
        if Pergunta.objects.filter(tema=tema).exists():
            ...
            msg = True
            return render(request, 'pedido/editar_tema.html', 
                        context={'erros': erros,'msg':msg,'tema':tema,})
        
        novo_tema = request.POST.get('novo_tema', '')
        
        tema_existente = Tema.objects.filter(tema=novo_tema).exclude(pk=pk).exists()
        if tema_existente:
            erros = "Já existe um ficheiro com esse nome"
            msg = True
            return render(request, 'pedido/editar_tema.html', 
                        context={'erros': erros,'msg':msg,'tema':tema,})
        ...
        tema.save()
        temas = Tema.objects.all()
        return render(request, 'pedido/listar_temas.html', {'temas': temas})

    # If it's a GET request, just render the template with the current tema
    return render(request, 'pedido/editar_tema.html', {'tema': tema})

def arquivar_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)

    # Verifique se o questionário possui perguntas com tema_id=7
    if Pergunta.objects.filter(questionario=questionario, tema_id=7).exists():
        estado_arquivado = get_object_or_404(EstadoQuestionario, estado="Arquivado")
        questionario.estado = estado_arquivado  # Define o estado como "Arquivado"
        questionario.save()
...
def publicar_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)

    questionario.save()
    return redirect('/../listar-questionarios/')

def inativar_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)
...
def excluir_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)
    respostas_existem = Resposta.objects.filter(pergunta__questionario=questionario).exists()
    confirmar_respostas = request.POST.get('confirmar_respostas') == 'true'

    if request.method == 'POST':
        if 'confirmar' in request.POST:
            if respostas_existem and not confirmar_respostas:
                return render(request, 'pedido/eliminar_questionario.html', {
                    'questionario_id': questionario.pk,
                    'respostas_existem': True,
                })
            else:
                if confirmar_respostas:
                    Resposta.objects.filter(pergunta__questionario=questionario).delete()
                questionario.delete()
                return redirect('/../listar-questionarios/')
        elif 'cancelar' in request.POST:
            return redirect('/../listar-questionarios/')
    ...
    return render(request, 'pedido/eliminar_questionario.html', {
        'questionario_id': questionario.pk,
        'respostas_existem': respostas_existem,
    })
...
def criar_estado_questionario(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/../listar-estados-questionarios/')
    else:
        form = EstadoQuestionarioForm()
    return render(request, 'pedido/criar_estado_questionario.html', {'form': form})


def listar_estados_questionario(request):
    estados = EstadoQuestionario.objects.all()

    # Verifica se há parâmetros de pesquisa na URL
    query = request.GET.get('q')
    if query:
        # Filtra os estados com base na pesquisa
        estados = estados.filter(Q(estado__icontains=query))

    return render(request, 'pedido/listar_estados_questionarios.html', {'estados': estados})


def eliminar_estado_questionario(request, estado_id):
    estado = get_object_or_404(EstadoQuestionario, pk=estado_id)
    ...
    return render(request, 'pedido/eliminar_estado_questionario.html', {'estado': estado})

def editar_estado_questionario(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('pedidos:listar_estados_questionario')
    else:
        form = EstadoQuestionarioForm(instance=estado)
    
    return render(request, 'pedido/editar_estado_questionario.html', {'form': form})


class ArquivarPedido(View):
    def get(self, request, pedido_id):
        # Verifica se o pedido existe
        pedido = get_object_or_404(Pedido, id=pedido_id)

        # Renderiza o template para confirmar o arquivamento
        return render(request, 'pedido/arquivar_pedido.html', {'pedido': pedido})
    ...
    def post(self, request, pedido_id):
        ...
        if 'confirmar' in request.POST:
            # Altera o estado do pedido para arquivado apenas se o botão "Sim" foi clicado
            pedido.estado_0 = EstadoPedido.objects.get(id=6)  # Supondo que o estado "arquivado" tenha o ID 6
            pedido.save()

        return redirect('/../listagempedidos')  # Redireciona de volta para a página de listagem de pedidos
    
    
    
class DesarquivarPedido(View):
    def get(self, request, pedido_id):
        # Verifica se o pedido existe
        pedido = get_object_or_404(Pedido, id=pedido_id)

        # Renderiza o template para confirmar o desarquivamento
        return render(request, 'pedido/desarquivar_pedido.html', {'pedido': pedido})
    ...
    def post(self, request, pedido_id):
        ...
        if 'confirmar' in request.POST:
            # Altera o estado do pedido para não arquivado apenas se o botão "Sim" foi clicado
            pedido.estado_0 = EstadoPedido.objects.get(id=1)  # Supondo que o estado "pendente" tenha o ID 1
            pedido.save()
...
def responder_questionario(request, questionario_id):
    questionario = get_object_or_404(Questionario, pk=questionario_id)
    if request.method == 'POST':
        return redirect('salvar_respostas')
    return render(request, 'pedido/responder_questionario.html', {'questionario': questionario})

def salvar_respostas(request):
    if request.method == 'POST':
        questionario_id = request.POST.get('questionario_id')
        questionario = get_object_or_404(Questionario, pk=questionario_id)
        ...
        return redirect('questionario_sucesso', questionario_id=questionario_id)
    return redirect('responder_questionario')

def questionario_sucesso(request, questionario_id):
    return render(request, 'pedido/questionario_sucesso.html', {'questionario_id': questionario_id})




####################################################################################################################
################### Incicio de Criar Pedido Aluno                 ####################################3333
####################################################################################################################    
def criar_pedidos_aluno(request):
    erros = ""
    msg = ""
    user = get_user(request)
    if user.groups.filter(name = "Docente").exists():
        if request.method == 'POST':
            ...
            if opcao == "1":
                if(subpedidos == []):
                    ...
                    msg = True
                    return render(request, 
                        template_name="pedido/criar_pedido_aluno.html",
                        context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos})
            elif opcao == "2":
                arquivo = request.FILES['arquivo']
                if str(arquivo).endswith(".xls") :
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    erros = "Já existe um pedido com esta data e assunto."
                    msg = True
                    return render(request, template_name="pedido/criar_pedido_aluno.html",
                        context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos})
                    ...
                    pedido.data_de_alvo = data
                    pedido.identificador_id = IdentificadorPedido.objects.get(id=opcao)
                    pedido.tipo = TipoDePedido.objects.get(id=5)
                    pedido.assunto = assunto
                    pedido.anoletivoid = ano_letivo.objects.get(ativo="S")
                    pedido.informacoes = descricao 
                    pedido.save()   
                    ...
                    funcionarios = Funcionario.objects.all()
                    mensagem_para_email = "O docente " + str(user) + " registou um novo Pedido de Aluno."
                    for x in funcionarios:
                        user_recipient = x
                        info = InformacaoMensagem(data=timezone.now(), pendente=True, titulo = "Registo de novo pedido de Aluno",
                                        descricao =mensagem_para_email, emissor = user , recetor = user_recipient, tipo = "Grupo" , lido = False)
                        info.save()
                        if user_recipient.id != user.id:
                            tmp = MensagemRecebida(mensagem=info)
...
####################################################################################################################
################### Fim de Criar Pedido Aluno                 ##################################################
####################################################################################################################
def consultar_pedido_de_aluno(request,id):
    pedido = Pedido.objects.get(id=id);
    pedido.estado = pedido.estado_0.estado
    pedido.data_de_alvo = pedido.data_de_alvo.strftime('%d/%m/%Y');
    file_url = ""
    if pedido.identificador_id.id == 1:
        ...
        pedido.fire_url = file_url
    return render(request, 
                        'pedido/consultar_pedido_aluno.html',
                        context={'pedido_outros': pedido, 'file_url': file_url })

def eliminar_pedido_de_aluno(request, id):
    pedido = Pedido.objects.get(id=id)
    user = get_user(request)
    if user.groups.filter(name = "Docente").exists():
        if pedido.estado_0.estado == 'Pendente':
            if request.method == 'POST':
                ...
                return render(request, 'pedido/eliminar_pedido_aluno.html', {'pedido_id': id})
        else:
            return redirect('main:mensagem',7)
    elif user.groups.filter(name = "Funcionario").exists():
        return redirect('main:mensagem',8)
####################################################################################################################
################### Incicio de Criar Pedido de Parecer                 ####################################3333
####################################################################################################################    
def criar_pedidos_parecer(request):
    erros = ""
    msg = ""
    user = get_user(request)
    if user.groups.filter(name = "Docente").exists():
        if request.method == 'POST':
            ...
            if opcao == "1":
                if(subpedidos == []):
                    ...
                    msg = True
                    return render(request, 
                        template_name="pedido/criar_pedido_parecer.html",
                        context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos})
            elif opcao == "2":
                arquivo = request.FILES['arquivo']
                if str(arquivo).endswith(".xls") :
            ...
            if form.is_valid():
                ...
                try:
                    ...
                    erros = "Já existe um pedido com esta data e assunto."
                    msg = True
                    return render(request, template_name="pedido/criar_pedido_parecer.html",
                        context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos})
                    ...
                    pedido.data_de_alvo = data
                    pedido.identificador_id = IdentificadorPedido.objects.get(id=opcao)
                    pedido.tipo = TipoDePedido.objects.get(id=6)
                    pedido.assunto = assunto
                    pedido.anoletivoid = ano_letivo.objects.get(ativo="S")
                    pedido.informacoes = descricao 
                    pedido.save()   
...
####################################################################################################################
################### Fim de Criar Pedido de Parecer               ##################################################
####################################################################################################################


def consultar_pedido_de_parecer(request,id):
    pedido = Pedido.objects.get(id=id);
    pedido.estado = pedido.estado_0.estado
    pedido.data_de_alvo = pedido.data_de_alvo.strftime('%d/%m/%Y');
    file_url = ""
    if pedido.identificador_id.id == 1:
        ...
        pedido.fire_url = file_url
    return render(request, 
                        'pedido/consultar_pedido_parecer.html',
                        context={'pedido_outros': pedido, 'file_url': file_url })


def eliminar_pedido_de_parecer(request, id):
    pedido = Pedido.objects.get(id=id)
    user = get_user(request)
    if user.groups.filter(name = "Docente").exists():
        if pedido.estado_0.estado == 'Pendente':
            if request.method == 'POST':
                ...
                return render(request, 'pedido/eliminar_pedido_parecer.html', {'pedido_id': id})
        else:
            return redirect('main:mensagem',7)
    elif user.groups.filter(name = "Funcionario").exists():
        return redirect('main:mensagem',8)
...
def editar_pedidos_parecer(request, id):
    erros = ""
    msg = ""
    user = get_user(request)
    pedido = get_object_or_404(PedidoOutros, id=id)

    if user.groups.filter(name="Docente").exists():
        if request.method == 'POST':
            subpedidos = json.loads(request.POST['subpedidos'])
            form = PedidoOutrosForm(request.POST, instance=pedido)
            descricao = request.POST.get('informacoes', '')
            assunto = request.POST.get('assunto', '')
            data = request.POST.get('data', '')
            opcao = request.POST.get('selected_option')
            ...
            if opcao == "1":
                if not subpedidos:
                    ...
                    msg = True
                    return render(request, 
                        template_name="pedido/editar_pedido_parecer.html",
                        context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos, 'pedido': pedido})
            elif opcao == "2":
                arquivo = request.FILES.get('arquivo')
                if arquivo and not str(arquivo).endswith(".xls"):
                    erros = "Ficheiro têm de ser do tipo .xls"
                    msg = True
                    return render(request, 
                        template_name="pedido/editar_pedido_parecer.html",
            ...
            if form.is_valid():
                try:
                    existing_pedido = PedidoOutros.objects.get(docenteutilizadorid=request.user.id, data_de_alvo=data, assunto=assunto)
                    if existing_pedido.id != pedido.id:
                        erros = "Já existe um pedido com esta data e assunto."
                        msg = True
                        return render(request, template_name="pedido/editar_pedido_parecer.html",
                            context={'erros': erros,'msg':msg,'form':form,'pedidos':subpedidos, 'pedido': pedido})
                
                except PedidoOutros.DoesNotExist:
                    pedido = form.save(commit=False)
                    pedido.estado_0 = EstadoPedido.objects.get(id=1)
                    pedido.docenteutilizadorid = Docente.objects.get(utilizador_ptr_id=request.user.id)
                    pedido.data_de_submissao = datetime.date.today()
                    ...
                    if opcao == "2" and arquivo:
                        ...
                        Outros.objects.filter(pedido_ptr_id=pedido.id).delete()
                        for subpedido in subpedidos:
                            outros = Outros(Assunto=subpedido['assunto'],
                                            Descricao=subpedido['descricao'],
                                            pedido_ptr_id=pedido.id)
                    ...
                    funcionarios = Funcionario.objects.all()
                    mensagem_para_email = "O docente " + str(user) + " editou um Pedido de Aluno."
                    for x in funcionarios:
                        user_recipient = x
                        info = InformacaoMensagem(data=timezone.now(), pendente=True, titulo = "Edição de pedido de Aluno",
                                        descricao =mensagem_para_email, emissor = user , recetor = user_recipient, tipo = "Grupo" , lido = False)
                        info.save()
                        if user_recipient.id != user.id:
                            tmp = MensagemRecebida(mensagem=info)
                    ...
                    for pedido in pedidos:
                        if pedido.estado_0.id == 1:
                            num_pedidos += 1
                    msg_automatica = "Ainda têm " + str(num_pedidos-1) + " pedidos pela frente."
                    info = InformacaoMensagem(data=timezone.now(), pendente=False, titulo = "Pedido Editado com sucesso",
                                descricao = msg_automatica,  recetor = user, tipo = "Individual" , lido = False)
                    info.save()
                    tmp = MensagemRecebida(mensagem=info)
                    tmp.save()
            ...
            form = PedidoOutrosForm(instance=pedido)
            msg = False
        
        return render(request, 
                    template_name="pedido/editar_pedido_parecer.html",
                    context={'erros': erros,'msg':msg,'form':form, 'pedido': pedido, 'pedidos': pedido.outros_set.all()})
    else:
        return redirect('main:mensagem',222)

def editar_questionario(request, pk):
    questionario = get_object_or_404(Questionario, pk=pk)
    temas = Tema.objects.all()
    anos_letivos = ano_letivo.objects.all()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Captura e salva o ano letivo selecionado
            ano_letivo_id = request.POST.get('ano_letivo')
            if ano_letivo_id:
                questionario.id_ano_letivo = ano_letivo_id

            # Captura e salva o tema selecionado
            tema_id = request.POST.get('tema')
            if tema_id:
                tema = Tema.objects.get(id=tema_id)
                questionario.tema = tema

            questionario.save()

            # Atualiza as perguntas existentes
            for pergunta in questionario.perguntas.all():
                pergunta_index = f'pergunta_{pergunta.pk}'
                texto = request.POST.get(pergunta_index)
                tipo = request.POST.get(f'selected_option_{pergunta.pk}')
                opcao1 = request.POST.get(f'{pergunta_index}_opcao_1', None)
                opcao2 = request.POST.get(f'{pergunta_index}_opcao_2', None)
                opcao3 = request.POST.get(f'{pergunta_index}_opcao_3', None)
                opcao4 = request.POST.get(f'{pergunta_index}_opcao_4', None)
                opcao5 = request.POST.get(f'{pergunta_index}_opcao_5', None)
                opcao6 = request.POST.get(f'{pergunta_index}_opcao_6', None)
                ...
                pergunta.opcao6 = opcao6
                pergunta.save()

            # Remove perguntas deletadas
            perguntas_ids = [int(key.split('_')[1]) for key in request.POST if key.startswith('pergunta_') and key.split('_')[1].isdigit()]
            for pergunta in questionario.perguntas.all():
                if pergunta.pk not in perguntas_ids:
                    pergunta.delete()

            # Adiciona novas perguntas
            pergunta_index = questionario.perguntas.count() + 1
            while f'pergunta_{pergunta_index}' in request.POST:
                texto = request.POST.get(f'pergunta_{pergunta_index}')
                tipo = request.POST.get(f'selected_option_{pergunta_index}')
                opcao1 = request.POST.get(f'pergunta_{pergunta_index}_opcao_1', None)
            ...
            return redirect('pedidos:listar_questionarios')
        else:
            print(form.errors)
    else:
        form = QuestionarioForm(instance=questionario)

    return render(request, 'pedido/editar_questionario.html', {
        'form': form,
        'questionario': questionario,
        'temas': temas,
        'anos_letivos': anos_letivos
...
def listar_estados(request):
    ...
    if query:
        estados = EstadoPedido.objects.filter(estado__icontains=query)
    else:
        estados = EstadoPedido.objects.all()
    return render(request, 'pedido/listar_estados.html', {'estados': estados})


def criar_estado(request):
    if request.method == 'POST':
        form = EstadoPedidoForm(request.POST)
        if form.is_valid():
            nome_estado = form.cleaned_data.get('estado')
            estado_existente = EstadoPedido.objects.filter(estado=nome_estado).exists()

            if estado_existente:
                messages.error(request, "Já existe um estado com esse nome.")
                return render(request, 'pedido/criar_estado.html', {'form': form})
            else:
                form.save()
                return render(request, 'pedido/estado_criado_sucesso.html')
    else:
        form = EstadoPedidoForm()
    return render(request, 'pedido/criar_estado.html', {'form': form})

def editar_estado(request, pk):
    estado = get_object_or_404(EstadoPedido, pk=pk)
    if request.method == 'POST':
        novo_estado = request.POST.get('novo_estado', '')
        descricao_estado = request.POST.get('descricao_estado', '')
        estado_existente = EstadoPedido.objects.filter(estado=novo_estado).exclude(pk=pk).exists()

        if estado_existente:
            messages.error(request, "Já existe um estado com esse nome!")
        else:
            estado.estado = novo_estado
            estado.descricao = descricao_estado
            estado.save()
            return render(request, 'pedido/estado_editado_sucesso.html', {'pk': pk})

    return render(request, 'pedido/editar_estado.html', {'estado': estado})


def eliminar_estado(request, estado_id):
    estado = get_object_or_404(EstadoPedido, pk=estado_id)
    
    # Verificacao pedidos
    pedidos_associados = Pedido.objects.filter(estado_0=estado).exists()
    
    if pedidos_associados:
        messages.error(request, "Estado tem pedidos associados, não é possível eliminar!")
        return redirect('pedidos:listar_estados')
    
    if request.method == 'POST':
        estado.delete()
        messages.success(request, "Estado eliminado com sucesso!")
        return redirect('pedidos:listar_estados')
    else:
        return render(request, 'pedido/eliminar_estado.html', {'estado': estado})
