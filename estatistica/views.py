...
def estatisticas_gerais(request):
    ...
    ano_letivo_selecionado = request.POST.get('ano_letivo')
    ...
    estatisticas_questionarios = []

    for questionario in questionarios:
        estatisticas = []

        perguntas = Pergunta.objects.filter(questionario=questionario, tema_id=1)
        ...
        for pergunta in perguntas:
            ...
            for resposta in respostas:
                if pergunta.tipo == 'multipla':
                    contagem_respostas[resposta.valor] += 1
                else:
                    contagem_respostas[resposta.resposta_texto] += 1

            opcoes = [pergunta.opcao1, pergunta.opcao2, pergunta.opcao3, 
                      pergunta.opcao4, pergunta.opcao5, pergunta.opcao6]
            opcoes = [opcao for opcao in opcoes if opcao]

            estatisticas.append({
                'pergunta_texto': pergunta.texto,
                'contagem_respostas': dict(contagem_respostas),
                'opcoes': opcoes,
                'tipo': pergunta.tipo
            })
        
        estatisticas_questionarios.append({
            'questionario': questionario,
            'estatisticas': estatisticas
        })

    context = {
        'estatisticas_questionarios': estatisticas_questionarios,
        'anos_letivos': anos_letivos,
        'ano_letivo_selecionado': int(ano_letivo_selecionado) if ano_letivo_selecionado else None,
    }

    return render(request, 'estatistica_gerais.html', context)

def estatisticas_satisfacao(request):
    anos_letivos = ano_letivo.objects.all()
    ano_letivo_selecionado = request.POST.get('ano_letivo')
    ...
    for questionario in questionarios:
        ...
        perguntas = Pergunta.objects.filter(questionario=questionario, tema_id=10)
    ...
    return render(request, 'estatistica_satisfacao.html', context)


def estatisticas_eficiencia(request):
    anos_letivos = ano_letivo.objects.all()
    ano_letivo_selecionado = request.POST.get('ano_letivo')
    ...
    for questionario in questionarios:
        ...
        perguntas = Pergunta.objects.filter(questionario=questionario, tema_id=11)
    ...
    return render(request, 'estatistica_eficiencia.html', context)



def estatisticas_funcionario(request):
    anos_letivos = ano_letivo.objects.all()
    ano_letivo_selecionado = request.POST.get('ano_letivo')
    ...
    for questionario in questionarios:
        ...
        perguntas = Pergunta.objects.filter(questionario=questionario, tema_id=2)
    ...
    return render(request, 'estatistica_funcionario.html', context)
